...
Lines is a list of terms F(Sj,Dj) or
...
if `true`, then a redundant algorithm using global reasoning is used to
...
If used, then the space in which the lines are placed should be thought of as
...
Rectangles is a list of terms F(Xj,Lj,Yj,Hj) or
...
A box is represented by a term \[Facet,Facet,...].
...
If `false` (the default), then the constraint is true iff, for all pairs of boxes i, j,
...
If `true`, then redundant reasoning methods are enabled, based on
...
If `true`, then cliques of objects are detected that clash in one
...
If `true`, then a redundant method is enabled that statically detects
...
If `true`, then a redundant reasoning method is enabled that solves a 2D
...
If `true`, and if all objects consist of a single shifted box of
...
If `true`, then the constraint that objects be non\-overlapping is
...
![images/geost1a](images/geost1a.png)
...
![images/geost1b](images/geost1b.png)
