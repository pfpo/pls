:- module(lists,[increasing_prefix/4,proper_suffix_length/3,
	nth1/4,group/4,rotate_list/3,
	head/2,nth1/3,delete/3,
	sublist/4,proper_length/2,exclude/4,
	append_length/3,clumped/2,sublist/5,
	shorter_list/2,nth0/3,ordered/2,
	rotate_list/2,proper_segment/2,ordered/1,
	exclude/5,maplist/2,some/3,
	nth0/4,convlist/3,group/5,
	select_max/4,delete/4,min_member/3,
	somechk/3,group/3,rev/2,
	perm2/4,suffix/2,maplist/4,
	decreasing_prefix/3,include/3,suffix_length/3,
	segment/2,permutation/2,proper_prefix_length/3,
	cons/3,map_product/4,somechk/4,
	keyclumps/2,prefix/2,is_list/1,
	scanlist/4,decreasing_prefix/4,append_length/4,
	select_max/3,maplist/3,nextto/3,
	select/3,perm/2,include/4,
	increasing_prefix/3,select_min/4,remove_dups/2,
	cumlist/4,some/4,max_member/3,
	subseq1/2,subseq/3,partition/5,
	cumlist/5,select_min/3,sumlist/2,
	proper_prefix/2,include/5,same_length/2,
	append/5,tail/2,reverse/2,
	transpose/2,one_longer/2,keyclumped/2,
	clumps/2,last/3,subseq0/2,
	last/2,some/2,exclude/3,
	selectchk/3,correspond/4,max_member/2,
	append/2,scanlist/5,scanlist/6,
	keys_and_values/3,selectchk/4,somechk/2,
	select/4,proper_suffix/2,cumlist/6,
	min_member/2,sublist/3,same_length/3,
	prefix_length/3]).
...
% ```
%
% @param KVS list of Key-Value pairs [K1-V1,...,Kn-Vn]
% @param KS list of Keys [K1,...,Kn]
% @param VS list of Values [V1,...,Vn]
...
%! cumlist(:Pred, +XS, ?V0, ?VS)
%! cumlist(:Pred, +XS, +YS, ?V0, ?VS)
%! cumlist(:Pred, +XS, +YS, +ZS, ?V0, ?VS)
%
...
%! scanlist(:Pred, +XS, ?V1, ?V)
%! scanlist(:Pred, +XS, +YS, ?V1, ?V)
%! scanlist(:Pred, +XS, +YS, +ZS, ?V1, ?V)
%
...
%
% @param Items list
% @param Counts list of Item-Count pairs
...
% been sound. In fact, it usually is the result of keysorting.
%
% @param Pairs list of Key-Item pairs
% @param Groups list of Key-Items pairs
