...
class PrologVisitor(TreeVisitor):
    ...
    def visit_functional_notation(self, node: Node, opts: Opts):
        ...
        match node.children:
            case [atom, _, arg_list, _]:
                ...
                return f
            case x:
                raise TypeError(f"Invalid shape of argument list: {node.children},Parent:\n{bytes.decode(node.text,'utf-8')}")
    ...
    def visit_operator_notation(self, node: Node, opts: Opts):
        ...
        match children:
            case [open, operand,close] if open.type == "open" and close.type == "close":
            ...
            case [op, operand] if op.type == "prefix_operator":
                ...
                return operand
            case _:
                raise TypeError(f"Unhandeled operator notation: {node.children},Parent:\n{bytes.decode(node.text,'utf-8')}")
