...
class OperatorDeclarationAnalysis(Analyser):
    ...
    def analyse_operator_decl(
        self, decl: OperatorDeclaration, content: PrologAnalyseable
    ):
    ...
    def analyse_precedence(self, decl: OperatorDeclaration, content: PrologAnalyseable):
        ...
        if (
            prec.isnumeric()
            and self.MIN_PRECEDENCE <= (prec_num := int(prec)) <= self.MAX_PRECEDENCE
        ):
            return prec_num
